...
Generics as a concept go far back in programming history, generally reknown as type parameterization. Recall the \ttt{ArrayList} data structure from the previous chapter on the Collections API. Imagine, for a moment, if the Java programmers had to write a differing implementation of \ttt{ArrayList} for \ttt{Integer}, \ttt{String}, \ttt{Double}, and so on ad infinitum. Not only is this impossible, it would also be extremely cumbersome and redundant, since the only altering parameter is the underlying element type in the data structure. Before Java 5, we could only use ``generics'' via collections of type \ttt{Object}, since it is the root class object type, meaning any element could be stored in any type of collection.
...
class GenericMaxTester {
  ...
  void genericMaxTest() {
    ...
    List<Double> l3 = new ArrayList<>(List.of(500.0, 400.0, 300.0, Math.PI, 200.0));
...
class GenericMax {
  ...
  /**
   * Returns the maximum element in the list according to the compareTo implementation
