...
\begin{align*}
  T(n) &= 2^kT(2^k/2^k) + k\cdot\Theta(n)\\
       ...
       &= \lg(n)\Theta(1) + \lg(n)\cdot Theta(n)\\
...
Finally, we will analyze the quick sort algorithm. We will analyze the randomized version of the algorithm, since it is more interesting and proves the efficacy of at least one randomized algorithm, which is a whole subfield in and of itself. 

In the best case, the pivot is always the median, indicating that half of the data is to either of its sides. \textbf{CONTINUE HERE}