...
\exercise{2}{chapter-crl}{The C programming language contains the \ttt{atoi} ``ascii-to-integer'' function, which receives a string and, if the string represents some integer, returns the number converted to an integer. Design the \ttt{int atoi(String s)} method that, when given a string $s$, returns its value as an integer if it can be parsed as an integer. If it cannot be parsed as an integer, or exceeds the bounds of an integer, return zero. Leading zeroes should be ignored, as well as leading and trailing space characters. Upon reading a non-digit after reading the first, return immediately with that value. E.g., \ttt{atoi("0043Hi")} returns \ttt{43}. Writing enough tests is \textit{crucial} to getting this correct.}
