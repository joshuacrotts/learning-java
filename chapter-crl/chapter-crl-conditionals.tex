...
class GpaCalculatorTesting {
  ...
  void gpaTest() {
    Assertions.assertAll(
      Assertions.assertEquals(4.0, gpa("A+")),
      Assertions.assertEquals(4.0, gpa("A")),
      Assertions.assertEquals(3.7, gpa("A-")),
      Assertions.assertEquals(3.3, gpa("B+")),
      Assertions.assertEquals(1.7, gpa("C-")),
      Assertions.assertEquals(0.7, gpa("D+")),
      Assertions.assertEquals(0.0, gpa("F+")),
      Assertions.assertEquals(0.0, gpa("F")),
      Assertions.assertEquals(0.0, gpa("F-"))
    );
  }
...
class GpaCalculator {
  ...
  static double gpa(String grade) {
    ...
    if (letter != 'A' && letter != 'F') {
      gpa = plus ? gpa + 0.3 : gpa;
    }
    // Compute - if applicable.
    if (letter != 'F') {
      gpa = minus ? gpa - 0.3 : gpa;
    }
...
\end{lstlisting}
\end{cl}
We evaluate $m$ inside the \ttt{switch} statement, and it resolves to one of twelve possible strings, assuming the input is a valid and capitalized month. If, for instance, the string is \ttt{"February"}, we assign $days$ to $28$ and perform a \ttt{break}. Cases that comprise a \ttt{switch} block can ``fall through'' to the next case, meaning that if we did not insert a \ttt{break}, the program would fall all the way to the \ttt{default} case and assign $31$ to $days$. Default cases correspond to ``anything other data'', similar to \ttt{else} blocks. In our case, we place all months that have thirty-one days in the default case to reduce the number of lines in our code. In the case of a month having thirty days, there are four possibilities, and we stack these atop one another to state that these months should have $30$ assigned to $days$. If we wanted to omit the \ttt{break} statements, we might instead inline \ttt{return} statements directly, since we do not do anything aside from return $days$ at the end of the method. Of course, this solution only works when the resulting target of a \ttt{switch} block \textit{is} the desired value.