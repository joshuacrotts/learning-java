...
\example Suit class example..., show how instantiating a type \ttt{Interface i = new Class()} is DIFFERENT from \ttt{Class c = new Class()}. The former is polymorphic, the latter is not, but the latter allows you to use methods defined ONLY in the class, whereas the former only allows you to use methods defined in the interface.
