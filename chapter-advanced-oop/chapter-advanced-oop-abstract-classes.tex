...
final class PrimNode extends AstNode {
  ...
  public Lvalue eval(Environment env) {
    ...
    return new Lvalue(
      switch (op) {
        case "+" -> this.add(operands);
        case "-" -> this.sub(operands);
        case "*" -> this.mul(operands);
        case "eq?" -> this.eq(operands);
        default -> null;
      });
  }
...
final class PrimNode extends AstNode {
  ...
  private AstNode add(List<Lvalue> operators) {
    return new NumberNode(operands.stream()
...
final class PrimNode extends AstNode {
  ...
  private AstNode sub(List<Lvalue> operators) {
    return new NumberNode(operands.stream()
                                  .map(Lvalue::getNumberValue)
...
final class PrimNode extends AstNode {
  ...
  private AstNode mul(List<Lvalue> operators) {
    return new NumberNode(operands.stream()
                                  .map(Lvalue::getNumberValue)
...
final class PrimNode extends AstNode {
  ...
  private AstNode eq(List<Lvalue> operators) {
    return new BooleanNode(operands.get(0).equals(operands.get(1)));
  }
}
\end{lstlisting}
\end{cl}
...
final class Environment {
  ...
  public AstNode lookup(String var) {
    if (this.ENV.containsKey(var)) { 
      ...
      return this.PARENT.ENV.lookup(var); 
    } else { 
      return null; 
    }
  }
