...
\begin{figure}[tp]
  ...
  \begin{tcolorbox}[title=Scanner Constructor Methods]
    ...
    \vspace{2ex}
  \begin{description}
    \item [\ttt{$S$ = new Scanner(System.in)}] creates a new scanner instance that reads from the standard input stream.
    \item [\ttt{$S$ = new Scanner($f$)}] creates a new scanner instance that reads from the file $f$, where $f$ is a \ttt{File} object.
...
\begin{figure}[tp]
  ...
  \begin{description}
    ...
    \item [\ttt{String $S$.nextLine()}] returns the next line from the scanner. The newline character is removed from the input buffer, but \textit{not} included in the returned string. 
  \end{description}
\end{tcolorbox}
...
class SudokuSolver {
  ...
  public void output(String filename) { /* TODO. */ }
}
\end{lstlisting}
\end{cl}
...
class SudokuSolver {
  ...
  public SudokuSolver(String filename) { /* Implementation omitted. */ }
  ...
  private boolean isValid(int r, int c, int n) {
    ...
    for (int i = 0; i < N; i++) {
      if (this.board[r][i] == n || this.board[i][c] == n) { return false; }
    }
