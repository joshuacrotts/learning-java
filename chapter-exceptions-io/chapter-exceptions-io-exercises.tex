...
\exercise{3}{chapter-arrays-collections}{The \ttt{cut} program is a command-line tool for extracting pieces of text from data. For this exercise, you will implement a very basic version of the program that reads data from the terminal.}

\begin{itemize}
    \item First, ... \ttt{-c X,Y,Z}
    \item ... \ttt{-c X-Y}
    \item ... \ttt{-c X-Y,Z-W}
    \item ... \ttt{-c 1-} end of line, \ttt{-b -5} up to fifth char
    \item ... \ttt{-dD} \ttt{-fX} flags. \ttt{-d$D$} means $D$ is a single-char delimiter, \ttt{-f$X$} means $X$ is either an interval or a range of fields to print. The fields are delimited by $D$. These cannot be used without the other.
\end{itemize}

\exercise{3}{chapter-arrays-collections}{The \ttt{sort} program is a command-line tool for sorting input from a data source. For this exercise, you will implement a very basic version of the program that reads data from the terminal.}
...
\begin{itemize}
    ...
    \item Add \ttt{-r} for reversed sorting.
    \item Add \ttt{-i} for case-insensitive sorting.
    \item Add \ttt{-c} for checking to see if a file is sorted. Reports the first occurrence of out-of-order sort.
    \item Add \ttt{-n} for sorting the values as if they are numbers. Notice the difference between sorting \ttt{9, 10, 8} with and without this flag.
    \item Add \ttt{-u} for removing duplicate values.
    \item Add \ttt{-o} for outputting to the file specified immediately after.
\end{itemize}
...
\exercise{3}{chapter-arrays-collections}{The \ttt{awk} program is a command-line tool for text parsing and processing. For this exercise, you will implement a very basic version of the program that reads data from the terminal. Be aware that this exercise is more in line with a mini-project.}
...
\begin{itemize}
    ...
    \item After getting the previous command to work, add support for conditional expressions. That is, suppose we want to print the second column of a row only if it has a value greater than $200$. We can achieve this via \ttt{"awk -h -F, '{print \$2(> \$2 200)}'}. If you \textit{really} want a challenge, you can add support for inlining other arithmatic expressions into a conditional. For example, if we want to print the third column only if the sum of the first two columns exceeds $1000$, we might write \ttt{awk -h -F, '{print \$3(> (+ \$1 \$2) 1000)}'}.
