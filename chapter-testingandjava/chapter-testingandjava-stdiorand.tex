...
\example{To generate a random integer between $-5$ and $15$, inclusive, using the \ttt{Math.random} method, we need to do something similar to our \ttt{Random} example. First, we multiply the result of \ttt{Math.random()} by $21$ to generate a floating-point value between $[0, 21)$. \textit{Casting}\index{casting}\index{cast}\index{type cast} (i.e., explicitly treating the returned expression as another type) this expression to an integer gives us an integer between $[0, 20]$. Finally, subtracting five gets us the desired range.}
\begin{verbnobox}[\small]
