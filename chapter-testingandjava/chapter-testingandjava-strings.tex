...
Comparing strings for equality seems straightforward: we can use \ttt{==} to compare one string versus another. Doing this is a common beginner pitfall! Strings are \textit{objects} and cannot be compared for value-equality using the \ttt{==} operator. Introducing this term ``value-equality'' insinuates that strings can, in fact, be compared using \ttt{==}, which is theoretically correct. The problem is that the result of this comparison only compares the \textit{hash codes} of the strings. In other words, \ttt{$s_1$ == $s_2$} returns whether the two strings reference the same string in memory. All objects, strings included, have a hash code, which (very) roughly corresponds to a location in memory; no two objects can have the same hash code. Two objects can, however, reference the same hash code. Let us see an example of what we mean.
...
In the above code snippet we declare $s_1$ as the string literal \ttt{"Hello"}, then initialize $s_2$ to point to $s_1$. So there are, in effect, two pointers to the string literal \ttt{"Hello"}. Let us try something a little more tricky: suppose we declare three strings, where $s_1$ is the same as before, $s_2$ is the string literal \ttt{"Hello"}, and $s_3$ is the string literal \ttt{"World"}. Comparing $s_1$ to $s_2$, strangely enough, also outputs true, but why? It seems that $s_1$ and $s_2$ reference different string literals, even though they contain the same characters. Indeed, this is the case, but Java performs an optimization called \textit{string pool caching}\index{string pool cache}. That is, if two strings are the same string literal, it makes little sense for them to point to two distinct hash codes, since strings are immutable. Therefore, Java optimizes these into references to the same string literal. Comparing $s_1$ or $s_2$ with $s_3$ outputs false, which is the anticipated result.
...
\begin{cl}[StringLexicographicallyEqual.java]{}
\begin{lstlisting}[language=MyJava]
...
%\begin{wrapfigure}[25]{r}[0.75in]{0.55\textwidth}
  ...
  \begin{description}
    ...
    \item[\ttt{char $S$.charAt($i$)}] retrieves the $i^\text{th}$ character in $S$.
    \item[\ttt{String $S$.substring($i$, $j$)}] returns a new string extracted from $S$, from indices $i$ inclusive to $j$ exclusive. The number of extracted characters is $j-i$.
    \item[\ttt{String $S$.substring($i$)}] returns a new string extracted from $S$, from index $i$ until the end of $S$.
    \item[\ttt{int $S$.indexOf($S'$)}] returns the index of the first occurrence of $S'$ in $S$, or $-1$ if $S'$ is not in $S$.
    \item[\ttt{boolean $S$.contains($S'$)}] returns \ttt{true} if $S'$ appears as a substring of $S$; \ttt{false} otherwise.
