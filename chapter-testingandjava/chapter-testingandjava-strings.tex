...
%\begin{wrapfigure}[25]{r}[0.75in]{0.55\textwidth}
  ...
  \begin{description}
    ...
    \item[\ttt{String $S$.substring($i$, $j$)}] returns a new string containing the characters from index $i$, inclusive, to index $j$, exclusive. The number of extracted characters is $j-i$.
    \item[\ttt{String $S$.substring($i$)}] returns a new string from index $i$ to the end of $S$.
    \item[\ttt{int $S$.indexOf($S'$)}] returns the index of the first instance of $S'$ in $S$, or $-1$ if $S'$ is not in $S$.
