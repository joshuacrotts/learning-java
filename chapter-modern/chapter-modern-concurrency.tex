...
\example{In the event that we want to block concurrent access to a variable, we can mark the method as \ttt{synchronized}, which means that only one thread can access the method at a time. Moreover, if we want to not deal with locks but still require atomic operations over access to an integer, we can use, for example, \ttt{AtomicInteger}. Let's revisit the \ttt{counter} example from earlier, but this time we will use an \ttt{AtomicInteger} to ensure that the increment operation is atomic.}
