...
\begin{cl}[DIntArray.java]{Dynamic Integer Array Method ``Constructors''}
...
\begin{cl}[DIntArrayTester.java]{Adding Value into Dynamic Array Tests}
\begin{lstlisting}[language=MyJava]
...
\begin{cl}[DIntArray.java]{Resize Array Method Implementation}
\begin{lstlisting}[language=MyJava]
class DIntArray {
...
\begin{cl}[DIntArray.java]{Adding Value into Dynamic Array Implementation}
\begin{lstlisting}[language=MyJava]
class DIntArray {
...
\begin{cl}[DIntArray.java]{``Get From List'' Dynamic Array Implementation}
\begin{lstlisting}[language=MyJava]
class DIntArray {
...
\begin{cl}[DIntArray.java]{``Set In List'' Dynamic Array Implementation}
\begin{lstlisting}[language=MyJava]
class DIntArray {
...
%\begin{wrapfigure}[25]{r}[0.75in]{0.55\textwidth}
  ...
  \begin{tcolorbox}[title=Java Array Lists]
    ...
    % \vspace{2ex}
  \begin{description}
    \item [\ttt{ArrayList<$T$> $A=$ new ArrayList<>()}] creates an \ttt{ArrayList} of type $T$ named~$A$.
...
\example{If we declare an \ttt{ArrayList} of integers $\ttt{ls1}$, we can perform several operations to demonstrate our understanding.}
\begin{verbatim}
...
\begin{figure}[tp]
  ...
  \begin{tcolorbox}[title=Java Linked Lists]
    ...
    % \vspace{2ex}
  \begin{description}
    \item [\ttt{LinkedList<$T$> $A=$ new LinkedList<>()}] creates a \ttt{LinkedList} of type $T$ named~$A$.
    \item [\ttt{$A$.get$(i)$}] retrieves the element at index $i^{\text{th}}$ of $A$. We refer to this as position $i + 1$. 
    \item [\ttt{$A$.set$(i, v)$}] assigns $v$ to index $i$ of $A$.
    \item [\ttt{$A$.size$()$}] returns the number of logical elements in the list, i.e., the logical size.
  \end{description}
...
Like the operations on a stack, these are also constant-time, since we may always keep a reference to the front and rear elements of a stack. Queues share similar drawbacks to stacks in that elements are not randomly accessible, in that we only know what exists at the front of a queue. Figure~\ref{fig:printerqueue} demonstrates the task queue of a printer, which has a sequence of files to print one after the other.
...
\begin{figure}[tp]
  ...
  \begin{tcolorbox}[title=Java PriorityQueue]
    ...
    \vspace{2ex}
  \begin{description}
    \item [\ttt{PriorityQueue<$T$> $PQ=$ new PriorityQueue<>($c$)}] creates a \ttt{PriorityQueue} of type $T$, named $PQ$, with a \ttt{Comparator} $c$.
...
A \textit{comparator} is a way of comparing two arbitrary ``things'', whether these things are numbers (i.e., the wrapper classes), strings, or any other kind of object, we can define custom ways of comparing \textit{any} non-primitive datatype!
...
\begin{figure}[tp]
  ...
  \begin{description}
    \item [\ttt{Map<$K, V$> $M=$ new HashMap<>()}] creates a \ttt{HashMap} named $M$ whose keys are of type $K$ and whose values are of type $V$. Namely, the keys map to the values.
     ...
     \item [\ttt{$M$.get$(k)$}] returns the value associated with the key $k$ in $M$, or \ttt{null} if $k$ does not have a mapping.
    \item [\ttt{$S$.size$()$}] returns the number of logical elements in the set.
  \end{description}
\end{tcolorbox}
...
\begin{cl}[IteratorTester.java]{Testing an Iterator over a \ttt{LinkedHashSet}}
\begin{lstlisting}[language=MyJava]
import static Assertions.assertAll;
import static Assertions.assertEquals;
...
\begin{cl}[EnhancedForLoopTester.java]{Using an Enhanced \ttt{for} Loop over a Collection}
\begin{lstlisting}[language=MyJava]
import static Assertions.assertEquals;
...
\begin{figure}[tp]
  ...
  \begin{tcolorbox}[title=Java Streams]
    ...
    \vspace{2ex}
  \begin{description}
    \item [\ttt{$S$.count$()$}] returns the number of elements in the stream.
    \item [\ttt{$S$.map($f$)}] returns a new stream whose elements are the result of applying $f$ to each element of $S$.
    \item [\ttt{$S$.filter($p$)}] returns a new stream whose elements are those of $S$ that satisfy the predicate $p$.
    \item [\ttt{$S$.reduce($a$, $f$)}] returns the result of applying the binary function $f$ to each element of $S$ and the accumulator $a$. The value of $a$ serves as the accumulator's initial value.
    \item [\ttt{$S$.limit($n$)}] returns a new stream whose elements are the first $n$ elements of $S$.
    \item [\ttt{$S$.skip($n$)}] returns a new stream whose elements are those of $S$ after the first $n$ elements.
    % \item [\ttt{$S$.anyMatch($p$)}] returns \ttt{true} if \textbf{at least one} element of $S$ satisfies the predicate $p$; otherwise, returns \ttt{false}.
    % \item [\ttt{$S$.allMatch($p$)}] returns \ttt{true} if \textbf{all} elements of $S$ satisfy the predicate $p$; otherwise, returns \ttt{false}.
    % \item [\ttt{$S$.noneMatch($p$)}] returns \ttt{true} if \textbf{no} elements of $S$ satisfy the predicate $p$; otherwise, returns \ttt{false}.
    \item [\ttt{$S$.min/max($c$)}] returns the minimum/maximum element of $S$ according to the comparator $c$. If $S$ is empty, returns \ttt{Optional.empty()}.
  \end{description}
\end{tcolorbox}
...
\example{Let's write the \ttt{squareList} method that receives a \ttt{List<Integer>} and squares each element using the Stream API. The method should return a new list. A motif presented throughout stream methods is that they do not modify the original data. We should use \ttt{map} to apply a lambda expression that receives an integer and returns its square. Fortunately for us, we can convert any collection into a stream using the \ttt{.stream()} method. From there, we use a simple \ttt{map} invocation to arrive at our desired outcome.}
...
\begin{cl}[OptionalTester.java]{Using the Optional Wrapper Class}
\begin{lstlisting}[language=MyJava]
import static Assertions.assertAll;
import static Assertions.assertEquals;
