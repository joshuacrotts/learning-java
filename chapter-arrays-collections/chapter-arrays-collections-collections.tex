...
\begin{figure}[tp]
  ...
  \begin{description}
    ...
    \item [\ttt{Stream<$T$> $S$.filter($p$)}] returns a new stream whose elements are those of $S$ that satisfy the predicate $p$.
    \item [\ttt{Stream<$T$> $S$.reduce($a$, $f$)}] returns the result of applying the binary function $f$ to each element of $S$ and the accumulator $a$. The value of $a$ serves as the accumulator's initial value.
    \item [\ttt{Stream<$T$> $S$.limit($n$)}] returns a new stream whose elements are the first $n$ elements of $S$.
    \item [\ttt{Stream<$T$> $S$.skip($n$)}] returns a new stream whose elements are those of $S$ after the first $n$ elements.
