...
\exercise{1}{chapter-arrays-collections}{Design the \ttt{double sumYucky(ArrayList<Integer> vals)} method that returns the average of the numbers in the list with the following caveat: The number $9$ should not be counted towards the average, nor should the following number, should one exist.}
\par{
\begin{verbatim}
...
\exercise{1}{chapter-arrays-collections}{Using the Stream API, design the \ttt{List<Integer> squareAddTenOmit(List<Integer> l)} that, when given a \ttt{List<Integer>} $l$, returns a list of those numbers squared and adds ten to the result, omitting any of the resulting numbers that end in $5$ or $6$.}
