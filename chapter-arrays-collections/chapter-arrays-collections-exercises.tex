...
\exercise{2}{chapter-arrays-collections}{We consider a \textit{key string} to be the string obtained after alphabetizing the letters of a string. For example, the string \ttt{"deloop"} is a key string of the strings \ttt{"poodle"} and \ttt{"looped"}. Write the \ttt{static HashMap<String, List<String>> keyStringGroups(List<String> ls)} method, which maps all key strings to the strings in $\textit{ls}$ using the above criteria}

{\footnotesize
\begin{verbatim}
ls = ["ant", "introduces", "poodle", "tan", "looped", "discounter", 
      "nastier", "polled", "retains", "retinas", "reductions"]
