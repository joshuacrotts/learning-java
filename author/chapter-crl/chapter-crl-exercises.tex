...
\myexercise{1}{chapter-crl}{Design the \ttt{double getDiscount(double c, int age, boolean isStudent)} met\-hod that computes a discount for some item cost $c$ based on their age $\mathit{age}$ and student status. Your method should return the total cost of the item after applying the discount. Use the following criteria for computing the discount:}
\begin{itemize}
...
\newpage %ugh
\myexercise{2}{chapter-crl}{Design the \ttt{double cookingScore(String type, double oz, int costDollars, int costCents, boolean isAppealing)} method, which scores a culinary piece in a cooking contest. \textbf{The returned score is a value in the interval $[0, 10]$.}}
...
\begin{itemize}
    \item If the \ttt{type} is \ttt{"Cake"} or \ttt{"Pasta"}, the base score is 1. If the \ttt{type} is \ttt{"Burger"}, the base score is 0.5. If the \ttt{type} is \ttt{"Pie"}, the base score is 0.75. Any other \ttt{type} is an automatic zero.
    \item If the weight \ttt{oz} is less than 4, their (current) score is multiplied by 0.9. If $4 \leq \ttt{oz} \leq 20$, their (current) score is multiplied by $y$ such that $y=1/16\ttt{oz}+0.25$. Otherwise, their (current) score is multiplied by $0.2$. 
    \item The \emph{combined} price of the piece adds a fixed amount to the score up to a total of \ttt{\$5.00}. Anything beyond this subtracts that amount from the score. For example, if the combined cost of a piece is $\$1.25$, then its score is increased by $1.25$. On the other hand, if the combined cost of a piece is $\$6.75$, then its score is decreased by $\$1.75$.
    \item If the piece is appealing, add a constant factor of $1.5$ to the piece.
\end{itemize}
...
A \emph{parenthesized string} is a string enclosed by parentheses. For example, the string \ttt{"(abc)pqr(de)"} contains two parenthesized strings: \ttt{"abc"}, and \ttt{"de"}.
...
\begin{enumerate}[label=(\alph*)]
    \item \textit{(9 points)} Design the standard recursive \texttt{collectParenthesizedStrings} method, which receives a \ttt{String}~$S$ and returns a \ttt{List} of all the parenthesized strings of~$S$.
    \item Design the \texttt{collectParenthesizedStringsTR} method and its accompanying helper method \texttt{collectParenthesizedStringsTRHelper}. The former acts as the driver to the latter; the latter solves the same problem as \texttt{collectParenthesizedStrings} does, but it instead uses tail recursion. Remember to include the relevant access modifiers!
    \item Design the \texttt{collectParenthesizedStringsLoop} method, which solves the problem using either a \texttt{while} or \texttt{for} loop.
\end{enumerate}
