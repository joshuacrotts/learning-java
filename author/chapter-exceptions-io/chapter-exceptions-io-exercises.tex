...
\myexercise{2}{chapter-exceptions-io}{Design the \ttt{String[] readStrings()} method, which reads an indeterminate number of line-separated strings from standard input. You \emph{cannot} store the strings in a collection---that is, you can only use Java arrays. Remember how to dynamically ``resize'' an array? Return the array of strings that are read from standard input. Your array should not contain ``null slots'' at the end. You may assume that a blank line indicates no further input.}
...
This sentence should also be capitalized.
    ...
    \emph{Example Run.} Assuming \ttt{dictionary.txt} contains a list of words, if we invoke the method with \ttt{spellChecker("dictionary.txt", "file3a.in")}, and \ttt{file3a.in} contains the following:
...
\myexercise{2}{The Haskell programming language provides \emph{sum types}, also called disjoint unions.}
A sum type is a type~$T$ that stores differing types.
For example, \ttt{Either<L, R>} is a sum type that encapsulates one value that is either of type \ttt{L} or is of type \ttt{R}, called the ``left'' and ``right'' respectively.
Haskell programmers (or other functional programmers) frequently use \ttt{Either} to indicate failure and success outcomes, where the ``left'' is populated upon failure and ``right'' is populated upon success.
The common example is designing a function to safely divide integers.
Namely, if the divisor is zero, it returns an \ttt{Either} whose ``left'' is a \ttt{String} with the value \ttt{"ERR / by zero"}, or something related.
Oppositely, if the divisor is not zero, then the division is declared ``safe,'' and the quotient is stored in the ``right'' value of the returned \ttt{Either}.
In this exercise you will implement the \ttt{Either} class to simulate Haskell's \ttt{Either} sum type.

\begin{enumerate}[label=(\alph*)]
    \item Design the generic \ttt{Either<L, R>} class. Its constructor should be private and receive values of types \ttt{L} and \ttt{R}, and assign them to instance variables.
    \item Design the \ttt{static <L, R> Either<L, R> left(L v)} and \ttt{static <L, R> Either<L, R> right(R v)} methods, which receive parameters of types \ttt{L} and \ttt{R} respectively. These two methods should invoke the \ttt{private} constructor and pass \ttt{null} for the object of the other type.
    \item Design the \ttt{isLeft()} and \ttt{isRight()} methods, which returns whether the \ttt{Either} represents a ``left'' or a ``right.''
    \item Override the \ttt{public String toString()} method to print out \ttt{"Left "} or \ttt{"Right "} followed by the stored value, depending on what value is stored.
    \item Now, design the \ttt{static <L, R> Either<L, R> safeDiv(int a, int b)} method, which returns an instance of \ttt{Either} based on the criteria in the preceding definition of \ttt{safeDiv}.
\end{enumerate}
...
\begin{enumerate}[label=(\alph*)]
    ...
    \item After getting the previous command to work, add support for inlined prefix operations in the \ttt{print} command. That is, suppose we want to print the sum of the second and fourth column of each row. To do this, we might write \ttt{awk -h -F, \q{}{print (+ \$2 \$4)}\q{} input.csv}. For simplicity, you may assume that there are only four operations: \ttt{\q{}+\q{}}, \ttt{\q{}-\q{}}, \ttt{\q{}*\q{}}, and \ttt{\q{}/\q{}}. 
    \item After getting the previous command to work, add multiple-argument support for \ttt{print}. That is, if we want to compute the product of the first three columns, output a string saying \ttt{" multiplied is " }, followed by the product, we could write \ttt{awk -h -F, \q{}{print \$1,\$2,\$3 " multiplied is " (* \$1 \$2 \$3) }\q{}}. Note that the delimiter between the column labels must match that passed by \ttt{-F}, otherwise there is no separator.
    \item After getting the previous command to work, add support for conditional expressions. That is, suppose we want to print the second column of a row only if it has a value greater than $200$. We can achieve this via \ttt{awk -h -F, \q{}{print \$2(> \$2 200)}\q{}}. If you \emph{really} want a challenge, you can add support for inlining other arithmetic expressions into a conditional. For example, if we want to print the third column only if the sum of the first two columns exceeds $1000$, we might write \ttt{awk -h -F, \q{}{print \$3(> (+ \$1 \$2) 1000)}\q{}}.
    \item Finally, add the \ttt{-v=N:V} flag that acts as a variable map that can be used in a print command. That is, suppose we want to create a variable called \ttt{val} and assign to it \ttt{30}. We can do this via \ttt{-v=val:30}, then reference it in a \ttt{print} via \ttt{\$}, e.g., \ttt{awk -F, \q{}{print \$val}\q{}}.
\end{enumerate}
