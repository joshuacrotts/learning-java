...
\myexample{Using \ttt{Random}, let's generate an integer between $0$ and $9$, inclusive on both bounds. To do so, we first need to instantiate a \ttt{Random} object, which we will call \ttt{random}. Then, we should invoke \ttt{nextInt} on the \ttt{random} object with an argument of $10$. Passing the argument $n$ to \ttt{nextInt} returns an integer $x \in [0, n-1]$.}
