...
\myexample{One final point to make about string equality is that string concatenation is not subject to the same optimization as string literals.} 
...
\myexample{What if we want to compare strings based on their character content rather than by reference?} 
...
For instance, \ttt{"hello".compareTo("hi")} returns \ttt{-4} because the distance from the first non-equal characters, those being \ttt{\q{}e\q} and \ttt{\q{}\q}, is minus four characters, since \ttt{\q{}i\q{}} is greater than \ttt{\q{}e\q{}}. 
