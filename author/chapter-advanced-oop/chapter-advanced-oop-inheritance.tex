...
\myexample{Let's design a ``role hierarchy'' system for business users. In this system, there are employees that have different roles. For instance, we have managers, developers, and salespersons. We have two types of developers: junior and senior. An employee may be promotable if it implements the \ttt{IPromotable} interface. First, let's consider the hierarchy and what properties each of these roles have to offer.}

An \ttt{Employee} contains a name and an identifier. \textbf{Continue here with HourlyEmployee and SalaryEmployee}
