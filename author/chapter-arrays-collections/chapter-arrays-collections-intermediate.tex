...
\begin{figure}[tp]
  %\begin{wrapfigure}[25]{r}[0.75in]{0.55\textwidth}
    ...
    \begin{description}
      ...
      \item [\ttt{BinarySearchTree<$T$> $t =$ new BinarySearchTree<>$(v, \mathit{cmp})$}] creates a binary search tree with a root of~$v$ and uses $\emph{cmp}$ as a comparator in the tree when inserting values.
