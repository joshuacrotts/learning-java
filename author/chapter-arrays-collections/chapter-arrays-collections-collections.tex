...
\myexample{One final example that we will consider is the \ttt{nthMostUniqueChar} method that, when given a string~$s$ and an integer~$n$, returns the~$n^\text{th}$ most unique character in the string, or \ttt{null} if there is no such character.} 
...
The most unique characters are \ttt{'a', 'b', 'c'}, so the third most unique character is \ttt{\q{}c\q{}}.

Another example is the string \ttt{"aabbcc"} and~$n=2$. 
The most unique characters are \ttt{'a', 'b', 'c'}, but all three have the same frequency of two, meaning they are all equally unique. 
In this case, we return the empty string.

A third example is the string \ttt{"aaaabccccc"} and~$n=2$. 
The most unique characters are \ttt{'c', 'a', 'b'}, so the second most unique character is \ttt{\q{}a\q{}}.
...
All of the Java collections implement \ttt{Iterator}, we can retrieve the corresponding \ttt{Iterator} object via the \ttt{.iterator} method. 
...
Should we want to traverse over the data again, we need to instantiate another instance of the iterator, because there is no way to reset the ``position'' of an iterator; they are ``one-time use'' objects.
