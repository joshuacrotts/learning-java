...
A slightly more compact solution would involve wrapping the conditional inside a call to \ttt{Math.max}, since the logic located within is effectively identical: \ttt{max = Math.max(arr[i], max)}.
