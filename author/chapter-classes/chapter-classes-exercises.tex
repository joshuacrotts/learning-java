...
\myexercise{1}{chapter-classes}{In this exercise you will create a small program to keep track of orders for a local pizzeria.}
\begin{enumerate}[label=(\alph*)]
  \item Design the \ttt{PizzaOrder} class, which stores a \ttt{Map<String, Integer>} as an instance variable. The map associates toppings (as strings) to their respective quantities on the pizza order. Toppings can be \ttt{"Pepperoni"}, \ttt{"Onion"}, \ttt{"Pineapple"}, or \ttt{"Anchovie"}. The \ttt{PizzaOrder} constructor should instantiate the map to a \ttt{LinkedHashMap}. As parameters to the constructor, it should receive two arrays: \ttt{String[] toppings} and \ttt{int[] toppingCount}. Insert each association into the map. You may assume that the input array parameters are of the same size. Be sure to also write the accompanying accessor method called \ttt{getToppings}.
  \item Design the \ttt{GroupOrder} class, which stores a \ttt{Queue<PizzaOrder>} as an instance variable. Its constructor instantiates the queue to a new priority queue, whose comparator is a parameter to the constructor. 
  \item Inside \ttt{GroupOrder}, design the \ttt{int addOrder(PizzaOrder po)} method, which adds the given \ttt{PizzaOrder} to the queue.
  \item Inside \ttt{GroupOrder}, design the \ttt{GroupOrder removeOrder(String t)} method, which returns a new \ttt{GroupOrder} whose \ttt{PizzaOrder} do not have the topping~\ttt{t}.
\end{enumerate}
...
\myexercise{2}{chapter-classes}{An earlier exercise asked you to design the \ttt{PizzaOrder} and \ttt{GroupOrder} classes. This exercise relies on the completion of that exercise, and introduces readers to the visitor design pattern, which we explore in greater detail in~\Cref{chapter-modern}.}
\begin{enumerate}[label=(\alph*)]
  \item Design the \ttt{ITopping} interface, which represents a topping. Then, design four classes: \ttt{Pepperoni}, \ttt{Onion}, \ttt{Pineapple}, and \ttt{Anchovie}, all of which implement \ttt{ITopping}.
  \item Now, let's design a class that allows us to do multiple actions with toppings. Design the generic \ttt{IToppingVisitor} class, which has four methods: \ttt{T visitPepperoni(Pepperoni p)}, \ttt{T visitOnion(Onion o)}, \ttt{T visitPineapple(Pineapple p)}, and \ttt{T visitAnchovie(Anchovie a)}.
  ...
  \item Finally, inside \ttt{GroupOrder}, design the \ttt{double cost(IToppingVisitor v)} method, which receives an instance of the \ttt{ToppingPriceVisitor} and returns the price of each \ttt{PizzaOrder}, giving their toppings.
\end{enumerate}
