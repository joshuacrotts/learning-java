...
So, let's design the \ttt{void addBook(Book b, int qty)}, \ttt{Book checkout(String title)}, \ttt{int getQuantity(String t)}, and \ttt{boolean containsBook(String t)} methods.
The tests will reuse the \ttt{Author} and \ttt{Book} declarations from the previous test.
