...
\myexample{Some readers may question why we emphasize mutation and aliasing. When working with the Collections framework and designing data structures, proper care must be taken to avoid undesired behavior and outcomes.} 
Consider what happens if we design a class \ttt{F} whose constructor receives a \ttt{List<Integer>}, which is assigned directly to an instance variable. 
Then, suppose we instantiate two distinct instances of \ttt{F}, namely~$f_1$ and~$f_2$, each of which receive the same (reference to a) list of numbers. 
...
This is a very inefficient way to store and lookup associations, at least when compared to data structures such as a \ttt{HashMap} or \ttt{TreeMap}. But, association lists are commonly used for adding (variable) bindings in a programming language, usually when writing simple interpreted languages.
