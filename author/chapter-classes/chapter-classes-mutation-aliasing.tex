...
The last assertion is the result of altering $D$'s reference. Recall that it initially references the new integer array of seven zeroes. The~$D$ array becomes an alias to~$E$, which might suggest that changing $E$ also changes what $D$ points to, but this is not the case. When we instantiate~$E$ to point to the new array of the integers from 1 to 7, $D$ remains aliased to the array of seven zeroes that $E$ was instantiated to. Therefore the last assertion should be not equals.
