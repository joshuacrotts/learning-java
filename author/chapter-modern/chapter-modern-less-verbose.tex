...
Additionally, this change means that any methods that we want to reference/invoke within \ttt{main} (that are defined inside the respective class) do not need to be categorized as static. For instance, let's once again write the \ttt{double fToC(double f)} method, only this time, we will call it from inside the \ttt{main} method.
